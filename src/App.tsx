...
export default function App() {
  ...
  const [allUsers, setAllUsers] = useState<
    { user_id: string; email: string; rol: Rol | null }[]
  >([]);
  ...
  const handleAuth = async () => {
    ...
    try {
      if (authMode === 'login') {
        ...
        if (error) setAuthError(error.message);
        else setAuthError('Revisa tu email para confirmar el registro.');
      }
    } finally {
  ...
  const loadAll = async () => {
    ...
    const [p, s, t, pe, on, oe, oc, ur] = await Promise.all([
      ...
      supabase.from('usuarios_roles').select('*'),
    ]);
    setProjects(p.data || []);
    setStages(s.data || []);
    ...
    // Build user list: auth users merged with roles
    const roles = ur.data || [];
    // Get list of auth users via admin API (only works if RLS allows, otherwise use roles table)
    ...
    if (authUsers?.users) {
      const merged = authUsers.users.map((u: any) => {
        ...
        return { user_id: u.id, email: u.email || '', rol: r?.rol || null };
      });
      setAllUsers(merged);
    } else {
      // Fallback: show only users with roles assigned
      setAllUsers(
        roles.map((r: any) => ({
          user_id: r.user_id,
          email: r.email || '',
  ...
  const deleteProject = async (id: string) => {
    if (!isAdmin || !confirm('¿Eliminar este proyecto y todas sus tareas?'))
      return;
  ...
  const deleteStage = async (id: string) => {
    ...
    setTasks((prev) =>
      prev.map((t) => (t.sid === id ? { ...t, sid: null } : t))
    );
  ...
  const setUserRol = async (
    ...
    if (newRol === null) {
      ...
      setAllUsers((prev) =>
        prev.map((u) => (u.user_id === user_id ? { ...u, rol: null } : u))
      );
      ...
      await supabase
        ...
        .eq('user_id', user_id);
      setAllUsers((prev) =>
        prev.map((u) => (u.user_id === user_id ? { ...u, rol: newRol } : u))
      );
    } else {
      await supabase
        .from('usuarios_roles')
        .insert({ user_id, email, rol: newRol });
      setAllUsers((prev) =>
        prev.map((u) => (u.user_id === user_id ? { ...u, rol: newRol } : u))
      );
    }
  ...
  const stageTasks = (sid: string, pid: string) =>
    tasks.filter((t) => t.pid === pid && t.sid === sid);
  const projectStages = (pid: string) =>
    stages.filter((s) => s.pid === pid).sort((a, b) => a.order - b.order);
  const getTask = (id: string) => tasks.find((t) => t.id === id);
  const getProject = (id: string) => projects.find((p) => p.id === id);
  const getStage = (id: string) => stages.find((s) => s.id === id);
  const filteredTasks = () =>
    activeProj ? tasks.filter((t) => t.pid === activeProj) : tasks;

  const taskStatus = (t: Task) => {
    if (t.status === 'done') return 'done';
    if (t.status === 'prog')
      return t.end_date && todayDate() > parseDate(t.end_date)!
        ? 'late'
        : 'prog';
    return 'todo';
  };
  ...
  const badgeStyle = (t: Task) => {
    ...
    const map: any = {
      ...
      late: { bg: 'rgba(239,68,68,.15)', color: COLORS.red, label: 'Atrasado' },
      todo: {
  ...
  const ganttRange = useCallback(() => {
    const t0 = todayDate();
    if (ganttZoom === 'week')
      return { min: addDays(t0, -1), max: addDays(t0, 13) };
    if (ganttZoom === 'month')
      return { min: addDays(t0, -3), max: addDays(t0, 33) };
    if (ganttZoom === 'quarter')
      return { min: addDays(t0, -7), max: addDays(t0, 83) };
    if (ganttZoom === 'custom' && ganttCustomStart && ganttCustomEnd)
      return {
        min: parseDate(ganttCustomStart)!,
        max: parseDate(ganttCustomEnd)!,
      };
  ...
  const orgSaveNode = async (pid: string, nk: string, x: number, y: number) => {
    ...
    if (existing) {
      ...
      setOrgNodes((prev) =>
        prev.map((n) => (n.id === existing.id ? { ...n, x, y } : n))
      );
    } else {
      const { data: row } = await supabase
        .from('org_nodes')
        .insert({ pid, node_key: nk, x, y })
        .select()
        .single();
  ...
  const orgAddEdge = async (pid: string, from_key: string, to_key: string) => {
    const exists = orgEdges.some(
      (e) => e.pid === pid && e.from_key === from_key && e.to_key === to_key
    );
    if (exists) return;
    const { data: row } = await supabase
      .from('org_edges')
      .insert({ pid, from_key, to_key })
      .select()
      .single();
  ...
  const orgToggleCheck = async (tid: string, sid: string) => {
    ...
    if (existing) {
      ...
      await supabase
        ...
        .eq('id', existing.id);
      setOrgChecks((prev) =>
        prev.map((c) => (c.id === existing.id ? { ...c, done: newDone } : c))
      );
    } else {
      const { data: row } = await supabase
        .from('org_checks')
        .insert({ tid, sid, done: true })
        .select()
        .single();
  ...
  const duplicateStage = async (
    ...
    for (const t of src) {
      const exists = tasks.some(
        (x) => x.name === t.name && x.sid === toSid && x.pid === toPid
      );
      if (!exists)
        await upsertTask({
          pid: toPid,
    ...
    return (
      ...
      >
        ...
        >
          ...
          <h2 style={{ color: COLORS.text, fontSize: 18, marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {authError && (
              <div
                style={{
                  fontSize: 12,
                  color: authError.includes('email')
                    ? COLORS.green
                    : COLORS.red,
                }}
              >
    ...
    return (
      ...
      >
        ...
        >
          ...
          <h2 style={{ color: COLORS.text, fontSize: 16, marginBottom: 10 }}>
            Acceso pendiente
          </h2>
          <p
            style={{
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: 10, color: COLORS.text3, marginTop: 4 }}>
            {user.email} · <span style={{ color: COLORS.amber }}>{rol}</span>
          </div>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
          ...
          {projects.map((p) => (
            <SidebarItem
              key={p.id}
              active={activeProj === p.id}
              onClick={() => setActiveProj(p.id)}
              color={p.color}
              label={p.name}
              count={projectTasks(p.id).length}
        ...
        >
          ...
          >
            <button style={btnStyle('sm')} onClick={handleLogout}>
              ⎋ Cerrar sesión
            </button>
          </div>
        </div>
      ...
      >
        ...
        >
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 12,
                  cursor: 'pointer',
                  background: currentTab === tab ? COLORS.bg : 'transparent',
                  color: currentTab === tab ? COLORS.text : COLORS.text2,
                  fontFamily: 'inherit',
                }}
              >
                {
          ...
          {canEdit && activeProj && (
            <button
              style={btnStyle('sm')}
              onClick={() => setModal({ type: 'stage', pid: activeProj })}
            >
              ⧉ Etapas
            </button>
          )}
  ...
  function GanttView() {
    ...
    useEffect(() => {
      const bl = bodyLeftRef.current,
        br = bodyRightRef.current,
        hr = headerRightRef.current;
      if (!bl || !br) return;
      const syncLR = () => {
        br.scrollTop = bl.scrollTop;
      };
      const syncRL = () => {
        bl.scrollTop = br.scrollTop;
        if (hr) hr.scrollLeft = br.scrollLeft;
      };
      bl.addEventListener('scroll', syncLR);
      br.addEventListener('scroll', syncRL);
      br.scrollLeft = Math.max(0, todayOffset - 150);
      return () => {
        bl.removeEventListener('scroll', syncLR);
        br.removeEventListener('scroll', syncRL);
      };
    ...
    while (cur <= max) {
      const label = cur.toLocaleDateString('es-CL', {
        month: 'short',
        year: '2-digit',
      });
      let days = 0;
      const m = cur.getMonth(),
        y = cur.getFullYear();
      while (cur <= max && cur.getMonth() === m && cur.getFullYear() === y) {
        days++;
        cur = addDays(cur, 1);
      }
    ...
    const projs = activeProj
      ? projects.filter((p) => p.id === activeProj)
      : projects;
    ...
    projs.forEach((p) => {
      ...
      const projPct = pt.length
        ? Math.round(pt.reduce((a, t) => a + t.pct, 0) / pt.length)
        : 0;
      ...
      pt.forEach((t) => {
        if (t.start_date) {
          const td = parseDate(t.start_date)!;
          if (!pMin || td < pMin) pMin = td;
        }
        if (t.end_date) {
          const td = parseDate(t.end_date)!;
          if (!pMax || td > pMax) pMax = td;
        }
        if (t.real_start) {
          const td = parseDate(t.real_start)!;
          if (!rMin || td < rMin) rMin = td;
        }
        const re = t.real_end
          ? parseDate(t.real_end)
          : t.real_start
          ? todayDate()
          : null;
        ...
        >
          ...
          {pMin && pMax && (
            <div
              style={{
                position: 'absolute',
                left: daysBetween(min, pMin) * DAY_W,
                top: 14,
                height: 6,
                width: daysBetween(pMin, pMax) * DAY_W + DAY_W,
                background: p.color,
                opacity: 0.3,
                borderRadius: 3,
              }}
          ...
          {rMin && rMax && (
            <div
              style={{
                position: 'absolute',
                left: daysBetween(min, rMin) * DAY_W,
                top: 22,
                height: 6,
                width: daysBetween(rMin, rMax) * DAY_W + DAY_W,
                background: p.color,
                opacity: 0.7,
                borderRadius: 3,
              }}
      ...
      if (!isCollapsed) {
        ...
        pStages.forEach((stage) => {
          ...
          const sPct = Math.round(
            st.reduce((a, t) => a + t.pct, 0) / st.length
          );
            ...
            >
              <span
                style={{
                  ...
                  color: COLORS.text3,
                  transition: 'transform 0.2s',
                  transform: isSC ? 'rotate(-90deg)' : 'none',
                }}
              >
                ▼
              </span>
          ...
          st.forEach((t) => {
            if (t.start_date) {
              const td = parseDate(t.start_date)!;
              if (!sMin || td < sMin) sMin = td;
            }
            if (t.end_date) {
              const td = parseDate(t.end_date)!;
              if (!sMax || td > sMax) sMax = td;
            }
          });
          rightRows.push(
            <div
            ...
            >
              ...
              {sMin && sMax && (
                <div
                  style={{
                    position: 'absolute',
                    left: daysBetween(min, sMin) * DAY_W,
                    top: 14,
                    height: 4,
                    width: daysBetween(sMin, sMax) * DAY_W + DAY_W,
                    background: p.color,
                    opacity: 0.2,
                    borderRadius: 2,
                  }}
          ...
          if (!isSC)
            st.forEach((t) =>
              renderTaskRow(t, p, min, totalW, todayOffset, leftRows, rightRows)
            );
        });
        unstaged.forEach((t) =>
          renderTaskRow(t, p, min, totalW, todayOffset, leftRows, rightRows)
        ...
        >
          ...
          {(['week', 'month', 'quarter', 'project'] as const).map((z) => (
            <button
              ...
              style={{
                ...
                fontSize: 10,
                fontFamily: 'monospace',
                background: ganttZoom === z ? COLORS.accent : COLORS.bg3,
                border: `1px solid ${
                  ganttZoom === z ? COLORS.accent : COLORS.border2
                }`,
                color: ganttZoom === z ? '#fff' : COLORS.text2,
                cursor: 'pointer',
              }}
            >
              {
          ...
          >
            <input
              ...
              onChange={(e) => {
                setGanttCustomStart(e.target.value);
                setGanttZoom('custom');
              }}
              style={{
                ...inputStyle,
            ...
            <input
              ...
              onChange={(e) => {
                setGanttCustomEnd(e.target.value);
                setGanttZoom('custom');
              }}
              style={{
                ...inputStyle,
          ...
          <button
            ...
            title="Mostrar/ocultar columnas de fecha"
            data-active={showDates}
          >
            📅 Fechas
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          >
            ...
            >
              ...
              {showDates && (
                <>
                  <ColHead w={colW.date}>Ini.Plan</ColHead>
                  <ColHead w={colW.date}>Fin Plan</ColHead>
                  <ColHead w={colW.date}>Ini.Real</ColHead>
                  <ColHead w={colW.date}>Fin Real</ColHead>
                </>
              )}
              <ColHead w={colW.status}>Estado</ColHead>
            </div>
            <div ref={bodyLeftRef} style={{ overflowY: 'auto', flex: 1 }}>
              {leftRows}
            </div>
          </div>
          {/* Right panel */}
                ...
                {days.map((d, i) => (
                  <div
                    ...
                    style={{
                      ...
                      fontSize: 9,
                      fontFamily: 'monospace',
                      color: d.isWeekend ? COLORS.text3 : COLORS.text2,
                      borderRight: `1px solid ${COLORS.border}`,
  ...
  function renderTaskRow(
        ...
        return (
          <div
            style={{
              ...
              fontSize: 10,
              fontFamily: 'monospace',
              color: late ? COLORS.red : val ? COLORS.text2 : COLORS.text3,
              padding: '2px 4px',
            }}
          >
            {display}
      ...
      if (editing)
        return (
          <input
            ...
            onBlur={async () => {
              setEditing(false);
              await upsertTask({ id: t.id, [field]: val2 || null });
            }}
            onKeyDown={(e) => {
      ...
      return (
        <div
          ...
          style={{
            ...
            fontFamily: 'monospace',
            color: late ? COLORS.red : val ? COLORS.text2 : COLORS.text3,
            fontStyle: !val && !isReal ? 'italic' : 'normal',
            padding: '2px 4px',
            cursor: 'pointer',
            borderRadius: 3,
          }}
    ...
    leftRows.push(
      <div
        key={`tr_${t.id}`}
        onDoubleClick={() => canEdit && setModal({ type: 'task', task: t })}
        style={{
          display: 'flex',
          alignItems: 'center',
          padding: '0 12px',
      ...
      >
        <div
          ...
          title={t.name}
        >
          <span style={{ color: COLORS.text3, marginRight: 4 }}>└</span>
          {t.name}
        </div>
        <div
          style={{
        ...
        >
          <span style={{ fontFamily: 'monospace', fontSize: 11, color: pc }}>
            {pct}%
          </span>
          <div
            style={{
          ...
          >
            {t.hh_real}/{t.hh_prog}h
          </span>
        </div>
        {showDates && (
          <>
            <DateCell field="start_date" val={t.start_date} isReal={false} />
            <DateCell field="end_date" val={t.end_date} isReal={false} />
            <DateCell field="real_start" val={t.real_start} isReal={true} />
            <DateCell field="real_end" val={t.real_end} isReal={true} />
          </>
        )}
    ...
    if (t.start_date && t.end_date) {
      ...
      const pw = Math.max(
        daysBetween(parseDate(t.start_date)!, parseDate(t.end_date)!) * DAY_W +
          DAY_W,
        DAY_W
      );
      bars.push(
    ...
    if (t.real_start) {
      ...
      const rc =
        s === 'done' ? COLORS.green : s === 'late' ? COLORS.red : p.color;
      bars.push(
        <div
          key="br"
          style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        ...
        >
          ...
          <StatCard
            val={`${totReal}h`}
            label="HH Reales"
            color={totReal > totProg ? COLORS.red : COLORS.green}
          />
          <StatCard
            val={`${totProg > 0 ? Math.round((totReal / totProg) * 100) : 0}%`}
            label="Uso HH"
            color={pctColor(
              totProg > 0 ? Math.round((totReal / totProg) * 100) : 0
            )}
          />
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {ft.map((t) => {
              ...
              const s = getStage(t.sid || '');
              const delta = t.hh_real - t.hh_prog;
              return (
                <tr
                  key={t.id}
  ...
  function ResumenView() {
    ...
    const totProg = ft.reduce((a, t) => a + t.hh_prog, 0);
    const totReal = ft.reduce((a, t) => a + t.hh_real, 0);
    const avgPct = ft.length
      ? Math.round(ft.reduce((a, t) => a + t.pct, 0) / ft.length)
      : 0;
    return (
      <div
        style={{
      ...
      >
        ...
        >
          ...
          {projects
            .filter((p) => !activeProj || p.id === activeProj)
            .map((p) => {
              const pt = projectTasks(p.id);
              if (!pt.length) return null;
              const pct = Math.round(
                pt.reduce((a, t) => a + t.pct, 0) / pt.length
              );
              return (
                <div
                  key={p.id}
  ...
  function CtrlView() {
    const todayStr = fmtDate(todayDate());
    const projs = ctrlFilterPid
      ? projects.filter((p) => p.id === ctrlFilterPid)
      : projects;
    const allTasks = ctrlFilterPid
      ? tasks.filter((t) => t.pid === ctrlFilterPid)
      : tasks;
    const lateTasks = allTasks
      .filter(
        (t) =>
          t.status !== 'done' &&
          ((t.end_date && t.end_date < todayStr) || expectedPct(t) - t.pct > 15)
      )
      .sort((a, b) =>
        (a.end_date || '9999') < (b.end_date || '9999') ? -1 : 1
      );
    ...
    return (
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <select
            value={ctrlFilterPid || ''}
            onChange={(e) => setCtrlFilterPid(e.target.value || null)}
        ...
        {lateTasks.length > 0 && (
          <Section
            title={`⚠ Tareas atrasadas o con avance insuficiente (${lateTasks.length})`}
          ...
          >
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {lateTasks.map((t) => {
                  const p = getProject(t.pid);
                  const stage = getStage(t.sid || '');
                  const delay =
                    t.end_date && t.end_date < todayStr
                      ? daysBetween(parseDate(t.end_date)!, todayDate())
                      : 0;
                  return (
                    <tr key={t.id}>
                      <td style={tdStyle}>
                      ...
                      <td style={tdStyle}>{stage?.name || 'Sin etapa'}</td>
        ...
        {projs.map((p) => {
          const pt = projectTasks(p.id);
          if (!pt.length) return null;
          const projPct = Math.round(
            pt.reduce((a, t) => a + t.pct, 0) / pt.length
          );
          const exp = Math.round(
            pt.reduce((a, t) => a + expectedPct(t), 0) / pt.length
          );
          const cum =
            exp > 0 ? Math.round((projPct / exp) * 100) : projPct > 0 ? 100 : 0;
          const pStages = projectStages(p.id);
          return (
            <Section
              key={p.id}
              ...
              rightEl={
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    >
                      {projPct}% real / {exp}% esperado → <b>{cum}% cumpl.</b>
                    </div>
                  </div>
                </div>
              }
            >
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {pStages.map((s) => {
                    const st = stageTasks(s.id, p.id);
                    if (!st.length) return null;
                    const sPct = Math.round(
                      st.reduce((a, t) => a + t.pct, 0) / st.length
                    );
                    const sExp = Math.round(
                      st.reduce((a, t) => a + expectedPct(t), 0) / st.length
                    );
                    const sCum =
                      sExp > 0
                        ? Math.round((sPct / sExp) * 100)
                        : sPct > 0
                        ? 100
                        : 0;
                    return [
                      ...
                      ...st.map((t) => (
                        <CtrlTaskRow key={t.id} t={t} indent={24} />
                      )),
                    ];
                  })}
                  {pt
                    .filter(
                      (t) => !t.sid || !pStages.find((s) => s.id === t.sid)
                    )
                    .map((t) => (
                      <CtrlTaskRow key={t.id} t={t} indent={10} />
                    ))}
                </tbody>
              </table>
            </Section>
  ...
  function CtrlTaskRow({ t, indent }: { t: Task; indent: number }) {
    const ep = expectedPct(t);
    const behind = ep - t.pct;
    const badge = badgeStyle(t);
    return (
      <tr>
        <td style={{ ...tdStyle, paddingLeft: indent, color: COLORS.text2 }}>
          {t.name}
        </td>
        <td style={tdStyle}>
          <MiniBar pct={t.pct} />
        ...
        <td
          style={{ ...tdStyle, fontFamily: 'monospace', color: COLORS.text3 }}
        >
          {ep}%
        </td>
        <td style={tdStyle}>
  ...
  function OrgView() {
    if (!activeProj)
      return (
        ...
        >
          Selecciona un proyecto para ver el organigrama.
        </div>
      );
    ...
    useEffect(() => {
      ...
      groups.forEach(([name, _], gi) => {
        ...
        pos[nk] = saved
          ? { x: saved.x, y: saved.y }
          : { x: 60 + (gi % 4) * 210, y: 60 + Math.floor(gi / 4) * 170 };
      });
      setPositions(pos);
    ...
    const drawEdges = () => {
      ...
      pEdges.forEach((e) => {
        const fp = positions[e.from_key],
          tp = positions[e.to_key];
        if (!fp || !tp) return;
        const fx = fp.x + 95,
          fy = fp.y + 80,
          tx = tp.x + 95,
        ...
        path.addEventListener('mouseenter', () =>
          path.setAttribute('stroke', COLORS.red)
        );
    ...
    const handleDrag = (nk: string, e: React.MouseEvent) => {
      ...
      const onUp = (ev: MouseEvent) => {
        ...
        const finalX = startPos.x + ev.clientX - startX,
          finalY = startPos.y + ev.clientY - startY;
    ...
    return (
      ...
      >
        ...
        >
          <span>
            Arrastra nodos · Click ↗ para conectar · Click en línea para
            eliminar
          </span>
          <button
            style={btnStyle('sm')}
            onClick={() => {
              if (activeProj) {
                setOrgNodes((prev) => prev.filter((n) => n.pid !== activeProj));
                supabase.from('org_nodes').delete().eq('pid', activeProj);
              }
            }}
          >
            ↺ Restablecer
        ...
        >
          <div
            style={{ position: 'relative', minWidth: 2000, minHeight: 1500 }}
          >
            <svg
              ref={svgRef}
              style={{
              ...
              {pEdges.map((e) => {
                ...
                return (
                  <path
                    ...
                    onClick={() => orgDeleteEdge(e.id)}
                    onMouseEnter={(ev) =>
                      (ev.target as SVGElement).setAttribute(
                        'stroke',
                        COLORS.red
                      )
                    }
              ...
              return (
                <div
                  ...
                  onMouseDown={(e) => {
                    if (
                      (e.target as HTMLElement).closest(
                        'button, span[data-check]'
                      )
                    )
                      return;
                    if (orgConnecting && orgConnecting !== nk) {
                      orgAddEdge(activeProj!, orgConnecting, nk);
                      setOrgConnecting(null);
                      return;
                    }
                    handleDrag(nk, e);
                  }}
                  style={{
                    ...
                    background: COLORS.bg3,
                    border: `1.5px solid ${
                      isConnSrc ? COLORS.green : COLORS.border2
                    }`,
                    borderRadius: 8,
                    cursor: 'grab',
                    userSelect: 'none',
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        if (orgConnecting === nk) setOrgConnecting(null);
                        else if (orgConnecting) {
                          orgAddEdge(activeProj!, orgConnecting, nk);
                          setOrgConnecting(null);
                        } else setOrgConnecting(nk);
                      }}
                      style={{
                        width: 18,
                        height: 18,
                        borderRadius: '50%',
                        background: isConnSrc ? COLORS.green : COLORS.bg4,
                        border: `1.5px solid ${
                          isConnSrc ? COLORS.green : COLORS.border2
                        }`,
                        color: isConnSrc ? '#000' : COLORS.text3,
                        fontSize: 11,
                        cursor: 'pointer',
                        display: 'flex',
                        alignItems: 'center',
                  ...
                  >
                    {pStages.map((s) => {
                      ...
                      const ck = orgChecks.find(
                        (c) => c.tid === inst.id && c.sid === s.id
                      );
                      ...
                      return (
                        <span
                          ...
                          style={{
                            ...
                            fontSize: 10,
                            fontFamily: 'monospace',
                            color: isDone ? COLORS.green : COLORS.text3,
                            background: isDone
                              ? 'rgba(62,207,142,0.1)'
                              : COLORS.bg4,
                            border: `1px solid ${
                              isDone ? 'rgba(62,207,142,0.25)' : 'transparent'
                            }`,
                            borderRadius: 3,
                            padding: '2px 5px',
                            cursor: 'pointer',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
        <div style={{ maxWidth: 700 }}>
          ...
          <p style={{ fontSize: 12, color: COLORS.text2, marginBottom: 20 }}>
            Asigna roles a los usuarios registrados. Los cambios tienen efecto
            inmediato.
          </p>
          ...
          >
            {[
              {
                rol: 'admin',
                desc: 'Acceso total — crea/elimina proyectos, etapas, gestiona usuarios',
              },
              {
                rol: 'editor',
                desc: 'Puede agregar/editar tareas, fechas y % avance',
              },
                    ...
                    <td style={tdStyle}>
                      <span
                        style={{
                          ...
                          borderRadius: 4,
                          border: `1px solid ${
                            u.rol ? `${rolColor(u.rol)}44` : COLORS.border
                          }`,
                        }}
                      >
                        {rolLabel(u.rol)}
                    ...
                    <td style={tdStyle}>
                      ...
                      >
                        {(['admin', 'editor', 'viewer'] as Rol[]).map((r) => (
                          <button
                            key={r}
                            onClick={() => setUserRol(u.user_id, u.email, r)}
                            style={{
                              padding: '3px 10px',
                              borderRadius: 4,
                              fontSize: 11,
                              ...
                              cursor: 'pointer',
                              border: `1px solid ${
                                u.rol === r ? rolColor(r) : COLORS.border2
                              }`,
                              background:
                                u.rol === r
                                  ? `${rolColor(r)}22`
                                  : 'transparent',
                              color: u.rol === r ? rolColor(r) : COLORS.text2,
                              fontWeight: u.rol === r ? 600 : 400,
                            }}
                          >
                            {r}
                          </button>
                        ...
                        {u.rol && u.user_id !== user?.id && (
                          <button
                            onClick={() => setUserRol(u.user_id, u.email, null)}
                            style={{
                              padding: '3px 8px',
                              borderRadius: 4,
                              fontSize: 11,
          ...
          <p style={{ fontSize: 11, color: COLORS.text3, marginTop: 12 }}>
            Los usuarios sin rol asignado pueden iniciar sesión pero no pueden
            ver ni modificar datos. Para ver nuevos registros, recarga la
            página.
          </p>
        </div>
      </div>
  ...
  function ModalRoot() {
    ...
    if (modal?.type === 'dupStage')
      return <DupStageModal fromSid={modal.fromSid} fromPid={modal.fromPid} />;
    return null;
  }
  ...
  function TaskModal() {
    ...
    const [form, setForm] = useState({
      pid: t?.pid || projects[0]?.id || '',
      sid: t?.sid || '',
      name: t?.name || '',
      owner: t?.owner || '',
      start_date: t?.start_date || '',
      end_date: t?.end_date || '',
      real_start: t?.real_start || '',
      real_end: t?.real_end || '',
      hh_prog: t?.hh_prog ?? 0,
      hh_real: t?.hh_real ?? 0,
      pct: t?.pct ?? 0,
    ...
    const stagesForPid = stages
      .filter((s) => s.pid === form.pid)
    ...
    const handleSave = async () => {
      await upsertTask({
        ...
        id: t?.id,
        sid: form.sid || null,
        real_start: form.real_start || null,
        real_end: form.real_end || null,
    ...
    return (
      <Overlay>
        <Modal title={t ? 'Editar tarea' : 'Nueva tarea'}>
          ...
          >
            <FormGroup label="Proyecto">
              <select
                value={form.pid}
                onChange={(e) =>
                  setForm((f) => ({ ...f, pid: e.target.value, sid: '' }))
                }
                style={inputStyle}
              >
                {projects.map((p) => (
            ...
            <FormGroup label="Estado">
              ...
              >
                ...
                <option value="prog">En curso</option>
                <option value="done">Completado</option>
              </select>
            </FormGroup>
            <FormGroup label="Inicio programado">
              <input
                type="date"
            ...
            <FormGroup label="Fin programado">
              <input
                type="date"
                value={form.end_date}
                onChange={(e) =>
            ...
            <FormGroup label="Inicio real">
              <input
                type="date"
                value={form.real_start}
                onChange={(e) =>
                  setForm((f) => ({ ...f, real_start: e.target.value }))
                }
                style={inputStyle}
              />
            </FormGroup>
            <FormGroup label="Fin real">
              <input
                type="date"
                value={form.real_end}
                onChange={(e) =>
                  setForm((f) => ({ ...f, real_end: e.target.value }))
                }
                style={inputStyle}
              />
            </FormGroup>
            <FormGroup label="HH programadas">
              <input
                type="number"
                value={form.hh_prog}
                min={0}
                step={0.5}
                onChange={(e) =>
                  setForm((f) => ({ ...f, hh_prog: +e.target.value }))
                }
                style={inputStyle}
              />
            </FormGroup>
            <FormGroup label="HH reales">
              <input
                type="number"
                value={form.hh_real}
                min={0}
                step={0.5}
                onChange={(e) =>
                  setForm((f) => ({ ...f, hh_real: +e.target.value }))
                }
                style={inputStyle}
              />
            </FormGroup>
          </div>
          <FormGroup label="% Avance (0–100)">
            <input
              type="number"
              value={form.pct}
              min={0}
              ...
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  pct: Math.min(100, Math.max(0, +e.target.value)),
                }))
              }
              style={inputStyle}
            />
          ...
          >
            ...
            <button style={btnStyle('sm')} onClick={() => setModal(null)}>
              Cancelar
            </button>
        ...
        <Modal title="Nuevo proyecto">
          ...
          <FormGroup label="Color">
            <input
              ...
              style={{ ...inputStyle, height: 38, padding: 3 }}
            />
          </FormGroup>
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <button
              style={btnStyle('primary')}
              onClick={async () => {
                await upsertProject({ name, color });
                setModal(null);
              }}
            >
              Crear
      ...
      <Overlay>
        <Modal title={`Etapas — ${p?.name || ''}`}>
          ...
          >
            {pStages.length === 0 && (
              <div
                style={{ color: COLORS.text3, fontSize: 12, padding: '8px 0' }}
              >
                Sin etapas aún
              </div>
            )}
            ...
            <>
              ...
              >
                ...
                <button
                  ...
                  onClick={async () => {
                    if (!name.trim()) return;
                    await upsertStage({ pid, name, order: pStages.length });
                    setName('');
                  }}
                >
                  Agregar etapa
  ...
  function DupStageModal({
    ...
    projects.forEach((proj) =>
      projectStages(proj.id).forEach((s) => {
        if (s.id !== fromSid)
          opts.push({
            sid: s.id,
            pid: proj.id,
            label: `${proj.name} → ${s.name}`,
          });
      })
    );
    return (
      <Overlay>
        <Modal title={`Copiar tareas de "${fromStage?.name || ''}"`}>
          <div style={{ fontSize: 11, color: COLORS.text3 }}>
            Las tareas se copian a la etapa destino (fechas reales y % se
            resetean). No se duplican si ya existen.
          </div>
          <FormGroup label="Etapa destino">
            ...
            >
              ...
              {opts.map((o) => (
                <option key={o.sid} value={`${o.sid}___${o.pid}`}>
                  {o.label}
                </option>
              ))}
            </select>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button
              style={btnStyle('sm')}
              onClick={() => setModal({ type: 'stage', pid: fromPid })}
            >
              Cancelar
            </button>
            <button
              ...
              onClick={async () => {
                if (!target) return;
                const [toSid, toPid] = target.split('___');
                await duplicateStage(fromSid, fromPid, toSid, toPid);
              }}
            >
              Copiar tareas
            </button>
    ...
    return (
      <Overlay>
        <Modal title="Responsables">
          ...
          <FormGroup label="Nuevo responsable">
            <input
              value={name}
              onChange={(e) => setName(e.target.value)}
              style={inputStyle}
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '7px 14px',
        cursor: 'pointer',
        background: active ? '#1e2333' : hover ? '#1e2333' : 'transparent',
    ...
    >
      ...
      <span style={{ fontSize: 10, fontFamily: 'monospace', color: '#5a6070' }}>
        {count}
      </span>
      {onDelete && hover && (
        <button
...
function LegendItem({ color, label }: { color: string; label: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
      <div
        style={{ width: 12, height: 5, borderRadius: 1, background: color }}
      />
      <span>{label}</span>
    </div>
  );
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 11, color: '#8b91a8', marginTop: 4 }}>
        {label}
      </div>
    </div>
  );
...
function FormGroup({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      <label
        style={{ fontSize: 11, color: '#8b91a8', fontFamily: 'monospace' }}
      >
        {label}
      </label>
      {children}
...
function MiniBar({ pct }: { pct: number }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <div
        style={{ width: 80, height: 6, background: '#252b3b', borderRadius: 3 }}
      >
        <div
          style={{
            width: `${pct}%`,
